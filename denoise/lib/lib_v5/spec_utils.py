...
def combine_spectrograms(specs, mp):
    ...
    l = min([specs[i].shape[2] for i in specs]) #频谱的最短长度

    spec_c = np.zeros(shape=(2, mp.param["bins"] + 1, l), dtype=np.complex64) #2表示两个声道

    offset = 0

    bands_n = len(mp.param["band"]) # band_n = 4

    for d in range(1, bands_n + 1):

        h = mp.param["band"][d]["crop_stop"] - mp.param["band"][d]["crop_start"] # 确定范围
    ...
    for d in range(1, bands_n + 1):
        ...
        if d == bands_n:  # higher
            ...
            if d == 1:  # lower
                ...
                if np.any(np.isinf(wave2)) or np.any(np.isnan(wave2)):
                    print("Audio data contains infinite or NaN values. Cleaning up...")
                    # 将无限大值或NaN值替换为0
                    wave2[np.isinf(wave2)] = 0
                    wave2[np.isnan(wave2)] = 0
